...
public class PlayerController : MonoBehaviour
{
    ...
    protected IEnumerator MoveForwards()
    {
        ...
        while (!arrived)
        {
            ...
            // Rotate the sphere according to the movement. @TODO This is not working properly yet. 
            Vector3 pos = Vector3.Scale(transform.position, world_direction);
            float rot = (((pos.x + pos.y + pos.z) % m_circum) * m_rotConst) - 180;
            player_sphere.transform.rotation = Quaternion.Euler(Vector3.Cross(world_direction * -rot, world_up));
    ...
    */
    protected IEnumerator MoveDownwards()
    {
        ...
        while (!arrived)
        {
            ...
            // Rotate the sphere according to the movement. @TODO refactor this here and in moveforwards
            Vector3 pos = Vector3.Scale(transform.position, world_direction);
        ...
        while (!arrived)
        {
            ...
            // Rotate the sphere according to the movement.  @TODO refactor 
            Vector3 pos = Vector3.Scale(transform.position, world_direction);
            float rot = (((pos.x + pos.y + pos.z) % m_circum) * m_rotConst) - 180.0f;
            player_sphere.transform.rotation = Quaternion.Euler(Vector3.Cross(world_direction * -rot, world_up));
